...
public class ai_healthbar : MonoBehaviour
{
    ...
    void OnGUI()
    {
        if (playing)
        {
            ...
            //GUI.Label(new Rect(screenW / 10, screenH / 10), "Health:");
            currentStyle.normal.background = healthTexture;
            GUI.Box(new Rect(gameObject.transform.position.x, gameObject.transform.position.y + 16/24, 24, 2), "", currentStyle);
            /*GUI.contentColor = Color.blue;
